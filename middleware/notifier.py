...
from system1 import get_mounted_filesystems, umount, get_sw_name
...
class notifier:
    ...
    def zpool_create(self, pool_name, disks):
        cmd = ''
        if len(disks) == 1:
            cmd = 'zpool create -o altroot=/mnt ' + pool_name + ' /dev/' + disks[0]
            print cmd
            self.__system(cmd)

    def zpool_status(self):
        return self.__pipeopen('zpool status').communicate()[0]

    def zpool_destroy(self, pool_name):
        self.__system('zpool destroy ' + pool_name)
